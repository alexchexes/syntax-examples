...
$patterns = [
    ...
    '/\\u{1F600}/u',   // PCRE sees:   /\u{1F600}/u    INVALID          /u enables UTF mode, not JavaScript-style \u{...}
    '/\\u1F600/',      // PCRE sees:   /\u1F600/       INVALID          PHP passes literal \u1F600; PCRE2 in PHP does not support \u...
    '/\\u1F600/u',     // PCRE sees:   /\u1F600/u      INVALID          same as above; /u does not make \u valid
    '/\\x{1F600}/',    // PCRE sees:   /\x{1F600}/     INVALID          PCRE2 sees a braced code point, but without /u 0x1F600 is too large for 8-bit mode
    '/\\x{1F600}/u',   // PCRE sees:   /\x{1F600}/u    VALID            PCRE2 braced hex escape in UTF mode; matches 😀
    '/\u{1F600}/',     // PCRE sees:   /\u{1F600}/     INVALID          single-quoted backslash stays literal; same runtime pattern as above
    '/\u{1F600}/u',    // PCRE sees:   /\u{1F600}/u    INVALID          single-quoted backslash stays literal; /u still does not make \u valid
    ...
    '/\x{1F600}/u',    // PCRE sees:   /\x{1F600}/u    VALID            single-quoted backslash stays literal; PCRE2 matches 😀 in UTF mode
    '/\x1F600/',       // PCRE sees:   /\x1F600/       VALID            PCRE2 reads \x1F, then literal 600; matches byte 0x1F + "600", not U+1F600
    '/\x1F600/u',      // PCRE sees:   /\x1F600/u      VALID            same bytes as above; /u does not change \x1F + "600"
    ...
    '/\u{4A}/u',       // PCRE sees:   /\u{4A}/u       INVALID          same as above; /u does not help
    ...
    *----------------------------*/
    ...
    '/\\u{1Q600}/u',   // PCRE sees:   /\u{1Q600}/u    INVALID          same as above; /u does not help
    ...
    '/\\x{1Q600}/u',   // PCRE sees:   /\x{1Q600}/u    INVALID          same as above; /u does not help
    ...
    '/\x1Q600/u',      // PCRE sees:   /\x1Q600/u      VALID            same bytes as above; /u does not change the parsed escape
    ...
    '/\u{4Q}/u',       // PCRE sees:   /\u{4Q}/u       INVALID          same as above
];