...
$patterns = [
    ...
    "/\x{1F600}/u",    // PCRE sees:   /\x{1F600}/u    VALID            PHP double quotes do not parse \x{...}; PCRE2 matches 😀 in UTF mode
    "/\x1F600/",       // PCRE sees:   /<0x1F>600/     VALID            PHP parses \x1F before PCRE; matches byte 0x1F + "600", not U+1F600
    "/\x1F600/u",      // PCRE sees:   /<0x1F>600/u    VALID            same bytes as above; /u does not change the parsed string
    ...
    "/\u{4A}/u",       // PCRE sees:   /J/u            VALID            PHP turns \u{4A} into literal J before PCRE; /u is harmless here
    ...
    *----------------------------*/
    ...
    "/\\u{1Q600}/u",   // PCRE sees:   /\u{1Q600}/u    INVALID          same as above
    "/\\u1Q600/",      // PCRE sees:   /\u1Q600/       INVALID          double-quoted \\ becomes one backslash, so PCRE2 still gets unsupported \u...
    "/\\u1Q600/u",     // PCRE sees:   /\u1Q600/u      INVALID          same as above
    "/\\x{1Q600}/",    // PCRE sees:   /\x{1Q600}/     INVALID          double-quoted \\ becomes one backslash, so PCRE2 sees non-hex Q in \x{...}
    "/\\x{1Q600}/u",   // PCRE sees:   /\x{1Q600}/u    INVALID          same as above
    
    "/\u{1Q600}/",     // PCRE sees:   (nothing)       PHP PARSE ERROR  invalid UTF-8 codepoint escape sequence in a double-quoted string
    "/\u{1Q600}/u",    // PCRE sees:   (nothing)       PHP PARSE ERROR  same as above; /u is never reached
    ...
    "/\x1Q600/u",      // PCRE sees:   /<0x01>Q600/u   VALID            same bytes as above; /u does not change the parsed string
    ...
    "/\u{4Q}/",        // PCRE sees:   (nothing)       PHP PARSE ERROR  invalid UTF-8 codepoint escape sequence in a double-quoted string
    "/\u{4Q}/u",       // PCRE sees:   (nothing)       PHP PARSE ERROR  same as above; /u is never reached
];